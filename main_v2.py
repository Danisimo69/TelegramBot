    ...
    if not await is_offer_defined(callback.from_user.id):
        ...
        await bot.send_photo(chat_id=user_id, photo=card.photo_id,
                       caption="✅ Пользователь ответил на ваше предложение обмена!\n"
                               ...
                               "С редкостью - " + get_rareness_by_num(card1.rareness), reply_markup=InlineButtons.insert_card_to_offer_kb())
...
async def penalti_game(callback: types.CallbackQuery):
    ...
    if callback.data == "pen_canc":
        ...
        await insert_lk_message_id(msg.message_id, res[0])
        return
    ...
    if finished:
        ...
        if game_res[0] == 0:
            ...
            draw_str = f"Результаты ударов @{await get_username_by_id(res[1])}:\n{scores[0]}\n" \
                       f"Результаты ударов @{await get_username_by_id(res[2])}:\n{scores[1]}\n" \
            ...
            fin_str = f"Результаты ударов @{await get_username_by_id(res[1])}:\n{scores[0]}\n" \
                      f"Результаты ударов @{await get_username_by_id(res[2])}:\n{scores[1]}\n" \
...
async def call_trade(callback: types.CallbackQuery):
    if callback.data == "trade_canc":
        ...
        await insert_lk_message_id(msg.message_id, trade_id[1])
...
async def get_buy_message(callback: types.CallbackQuery):
    if callback.data == "0":
        await check_pay(callback)
    else:
        ...
        redirect_uri = await quick_pay(
            "buy cards", await get_price(int(callback.data)), operation_id)
...
async def admin_execute(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "adm_add_card":
        ...
        await bot.edit_message_text(message_id=await get_lk_id_message(callback.from_user.id),
    ...
    if callback.data == "adm_del_promo":
        ...
        if promos != None:
            for i in range(0, len(promos[0])):
                promos_for_kb.append({"text": str(promos[0][i][1]) + " - " + promos[1][i][2],
            ...
            await insert_lk_message_id(msg.message_id, callback.from_user.id)
            return
        await callback.message.edit_text("Выберете промокод для удаления\nВсе промокоды указаны в формате промокод - карточка",
...
async def add_card_to_promo(callback: types.CallbackQuery):
    ...
    if callback.data.split("_")[2] == "del":
        ...
        await callback.message.edit_text("Промокод был успешно удален", reply_markup=AdminInlineKeyboard.adm_add_card_kb())
        return
...
async def check_promocode(message: types.Message, state: FSMContext):
    ...
    if state_ == UserState.check_promo.state:
        ...
        res = await check_promo_(message.chat.id, message.text)
    ...
    if state_ == UserState.get_second_user_for_offer.state:
        ...
        user_id = await search_user_by_username(username, message.chat.id)
        ...
        if user_id == -1:
            ...
            await insert_lk_message_id(msg.message_id, user_id)
    ...
    if state_ == UserState.get_username_for_pen.state:
        ...
        if tele_id2 == -1:
            ...
            if await check_delta_rating(message.chat.id, int(tele_id2)):
            ...
            if not await user_in_game(int(tele_id2)):
                await create_game(message.chat.id)
                await insert_second_user(tele_id2, message.chat.id)

                msg = await bot.send_message(tele_id2,
                                       "@" + await get_username_by_id(message.chat.id) + " предлагает вам сыграть в Пенальти!",
                ...
                await insert_lk_message_id(msg.message_id, message.from_user.id)

    if state_ == UserState.get_username_for_admin.state:

        user_id = await search_user_by_username(message.text, -1)
        if user_id == -1:
            await bot.send_message(message.chat.id, "Пользователь с таким именем не был найден. "
            ...
            msg = await bot.send_message(
                message.chat.id, text=user_str, reply_markup=AdminInlineKeyboard.get_username_for_admin_kb())
            await insert_lk_message_id(msg.message_id, message.chat.id)
    ...
    if state_ == UserState.save_card_photo.state:
        ...
        if (await set_card_photo(card_id, message.photo[0].file_id)) != -1:
            ...
            await bot.send_photo(message.from_user.id,
                           new_card[1], caption=new_card[0], reply_markup=AdminInlineKeyboard.set_card_photo_kb(card_id))
        else:

            msg = await bot.send_message(
                message.from_user.id, "Произошла какая-то ошибка, уже работаем над этим", reply_markup=AdminInlineKeyboard.get_photo_kb())
            await insert_lk_message_id(msg.message_id, message.from_user.id)
    ...
    if state_ == UserState.get_new_photo.state:
        ...
        if card is not None and await check_card_info(player_info) != -1:

            msg = await bot.send_message(
                message.from_user.id, "Теперь отправьте фото карточки игрока", reply_markup=AdminInlineKeyboard.get_new_photo_kb(False))
            await insert_lk_message_id(msg.message_id, message.from_user.id)
    ...
    if state_ == UserState.save_new_card_photo.state:
        ...
        if await set_card_photo(card_id, message.photo[0].file_id) != -1:
            ...
            new_card = await get_card_by_id(card_id)
            await bot.send_photo(message.from_user.id,
                           new_card[1], caption=new_card[0], reply_markup=AdminInlineKeyboard.save_new_card_photo_kb(True))
            msg = await bot.send_message(message.from_user.id, ".")
            await insert_lk_message_id(msg.message_id, message.from_user.id)
        else:

            msg = await bot.send_message(
                message.from_user.id, "Произошла какая-то ошибка, уже работаем над этим",reply_markup=AdminInlineKeyboard.save_new_card_photo_kb(False))
            await insert_lk_message_id(msg.message_id, message.from_user.id)
    ...
    while True:
        ...
        if games_list:
            for game in games_list:
                result = await destroy_game(game[0])
                if result[0] != 0:
